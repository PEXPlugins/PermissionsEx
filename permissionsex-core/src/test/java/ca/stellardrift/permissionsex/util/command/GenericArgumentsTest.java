...
public class GenericArgumentsTest {
    ...
    public void testSequence() throws ArgumentParseException {
        CommandElement el = GenericArguments.seq(GenericArguments.string(fixed("one")), GenericArguments.string(fixed("two")), GenericArguments.string(fixed("three")));
    ...
    public void testChoices() throws ArgumentParseException {
        CommandElement el = GenericArguments.choices(fixed("val"), ImmutableMap.of("a", "one", "b", "two"));
    ...
    public void testFirstParsing() throws ArgumentParseException {
        CommandElement el = GenericArguments.firstParsing(GenericArguments.integer(fixed("val")), GenericArguments.string(fixed("val")));
    ...
    public void testOptional() throws ArgumentParseException {
        CommandElement el = GenericArguments.optional(GenericArguments.string(fixed("val")));
        CommandContext context = parseForInput("", el);
        assertNull(context.getOne("val"));

        el = GenericArguments.optional(GenericArguments.string(fixed("val")), "def");
        context = parseForInput("", el);
        assertEquals("def", context.getOne("val"));

        el = GenericArguments.seq(GenericArguments.optionalWeak(GenericArguments.integer(fixed("val"))), GenericArguments.string(fixed("str")));
        context = parseForInput("hello", el);
        assertEquals("hello", context.getOne("str"));

        el = GenericArguments.seq(GenericArguments.optional(GenericArguments.integer(fixed("val")), GenericArguments.string(fixed("str"))));
    ...
    public void testRepeated() throws ArgumentParseException {
        CommandContext context = parseForInput("1 1 2 3 5", GenericArguments.repeated(GenericArguments.integer(fixed("key")), 5));
        assertEquals(ImmutableList.<Object>of(1, 1, 2, 3, 5), context.getAll("key"));
    }

    @Test
    public void testAllOf() throws ArgumentParseException {
        CommandContext context = parseForInput("2 4 8 16 32 64 128", GenericArguments.allOf(GenericArguments.integer(fixed("key"))));
    ...
    public void testString() throws ArgumentParseException {
        CommandContext context = parseForInput("\"here it is\"", GenericArguments.string(fixed("a value")));
        assertEquals("here it is", context.getOne("a value"));
    }

    @Test
    public void testInteger() throws ArgumentParseException {
        CommandContext context = parseForInput("52", GenericArguments.integer(fixed("a value")));
        assertEquals((Integer) 52, context.getOne("a value"));

        assertThrows(ArgumentParseException.class, () -> parseForInput("notanumber", GenericArguments.integer(fixed("a value"))));
    }

    @Test
    public void testUUID() throws ArgumentParseException {
        final UUID subject = UUID.randomUUID();
        CommandContext ctx = parseForInput(subject.toString(), GenericArguments.uuid(fixed("a value")));
        assertEquals(subject, ctx.getOne("a value"));


        assertThrows(ArgumentParseException.class, () -> parseForInput("48459", GenericArguments.uuid(fixed("a value"))));
        assertThrows(ArgumentParseException.class, () -> parseForInput("words", GenericArguments.uuid(fixed("a value"))));
    }

    @Test
    public void testBool() throws ArgumentParseException {
        CommandElement boolEl = GenericArguments.bool(fixed("val"));
    ...
    public void testEnumValue() throws ArgumentParseException {
        CommandElement enumEl = GenericArguments.enumValue(fixed("val"), TestEnum.class);
    ...
    public void testRemainingJoinedStrings() throws ArgumentParseException {
        CommandElement remainingJoined = GenericArguments.remainingJoinedStrings(fixed("val"));
