...
public abstract class PermissionEntity {
    ...
    public PermissionEntity(String name, PermissionManager manager) {
        ...
        Logger.getLogger("Minecraft").info("New instance - \"" + name + "\"");
    }
    ...
    protected String getMatchingExpression(String permission, String world) {
        for (String expression : this.getPermissions(world)) {
            if(getPattern(expression).matcher(permission).matches()){
                return expression;
            }
        }
    ...
    public static Pattern getPattern(String expression) {
        ...
        if (!patternCache.containsKey(expression)) {
            patternCache.put(expression, Pattern.compile(expression.replace(".", "\\.").replace("*", "(.*)")));
        }

        return patternCache.get(expression);
    }
}
